...
void AGeometryHubActor::Tick(float DeltaTime)
{
	...
	UE_LOG(LogGeometryHub,Error, TEXT("none property pointer %i, is valid %i "), NonePropertyActor!=nullptr, IsValid(PropertyActor));
...
void AGeometryHubActor::DoActorSpawn1()
{
	...
	if (World)
	{
		for (int32 i=0;i<10;++i)
		{
			int32 a=50;
			a=i*a;
			const FTransform GeometryTransform = FTransform(FRotator::ZeroRotator,FVector(0.0f,300.0f,300.0f+a) );
			ABaseGeometryActor* Geometry = World->SpawnActor<ABaseGeometryActor>(GeometryClass, GeometryTransform);
			
			if(Geometry)
			{
				FGeometryData Data;
				Data.MoveType = EMovementType::Sin;
				Data.Frequence = FMath::RandRange(0.1,10);
...
void AGeometryHubActor::DoActorSpawn2()
{
	...
	if (World)
	{
		for (int32 i=0;i<10;++i)
		{
			int32 a=50;
			a=i*a;
			const FTransform GeometryTransform = FTransform(FRotator::ZeroRotator,FVector(0.0f,0.0f,300.0f+a) );
	...
	if (World)
	{
		for (const FGeometryPayLoad PayLoad:GeometryPayLoad)
		{
			...
			if(Geometry)
			{
				...
				Geometry->OnTimeFinished.AddUObject(this,&AGeometryHubActor::OnTimeFinished);
...
{
	if(!GetWorld())return;
	FTransform GeometryTransform = FTransform(FRotator::ZeroRotator,FVector(0.0f,300.0f,300.0f) );
...
{
	UE_LOG(LogGeometryHub,Warning, TEXT("ACtors name: %s Color %s"), *Name, *Color.ToString());
}

void AGeometryHubActor::OnTimeFinished(AActor* Actor)
{
	if (!Actor) return;
	UE_LOG(LogGeometryHub,Error, TEXT("TimerFinished %s"), *Actor->GetName());

	ABaseGeometryActor* Geometry = Cast<ABaseGeometryActor>(Actor);
	if (!Geometry) return;
	UE_LOG(LogGeometryHub,Display, TEXT("Cast success"));
